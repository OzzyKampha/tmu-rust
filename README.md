...
- Save/load trained models and encoders to disk (`save`/`load`) — a compact, dependency-free binary format that preserves all learned state **and** RNG streams, so a reloaded model predicts identically and can resume training deterministically
